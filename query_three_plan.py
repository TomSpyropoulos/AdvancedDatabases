...
# Apply the BROADCAST join hint
df_hinted = df.hint("broadcast")
df_broadcast = df_hinted.join(revgeo_df, (df['LAT'] == revgeo_df['LAT']) & (df['LON'] == revgeo_df['LON']))
# Print the execution plan
df_broadcast.explain()

# Apply the MERGE join hint
df_hinted = df.hint("merge")
df_merge = df_hinted.join(revgeo_df, (df['LAT'] == revgeo_df['LAT']) & (df['LON'] == revgeo_df['LON']))
# Print the execution plan
df_merge.explain()

# Apply the SHUFFLE_HASH join hint
df_hinted = df.hint("shuffle_hash")
df_shuffle_hash = df_hinted.join(revgeo_df, (df['LAT'] == revgeo_df['LAT']) & (df['LON'] == revgeo_df['LON']))
# Print the execution plan
df_shuffle_hash.explain()

# Apply the SHUFFLE_REPLICATE_NL join hint
df_hinted = df.hint("shuffle_replicate_nl")
df_shuffle_replicate_nl = df_hinted.join(revgeo_df, (df['LAT'] == revgeo_df['LAT']) & (df['LON'] == revgeo_df['LON']))
# Print the execution plan
df_shuffle_replicate_nl.explain()
